...
import { Card, Row, Col, Button, Typography, Layout, Modal, Input, Select, Form, message, Slider } from 'antd';
import { 
  DatabaseOutlined, 
  MessageOutlined, 
  ExperimentOutlined,
...
export default function MainInterface({ user, onLogout }) {
  ...
  const handleConfigSubmit = async (values) => {
    ...
    try {
      ...
      message.error(`Failed to update configuration: ${err.response?.data?.error || err.message}`);
    } finally {
  ...
  const showConfigModal = async () => {
    ...
    try {
      ...
      configForm.setFieldsValue({
        system_prompt: dialog.system_prompt || `You are a knowledgeable and helpful chatbot specializing in Traditional Eastern Medicine.
  ...
  const renderHomeView = () => (
    <div style={{ padding: '24px' }}>
      <Title level={2} style={{ textAlign: 'center', marginBottom: '40px' }}>
        Chào mừng đến với Chatbot dành cho ngữ liệu về Y học cổ truyền phương Đông, {user}!
      </Title>
      ...
      <Row gutter={[24, 24]} justify="center">
        ...
        <Col xs={24} sm={12} md={8}>
          ...
          >
            ...
            <Text type="secondary">
              Quản lý dialogs và chat với AI
            </Text>
          </Card>
        </Col>
        ...
        <Col xs={24} sm={12} md={8}>
          ...
          >
            <ExperimentOutlined style={{ fontSize: '48px', color: '#fa8c16', marginBottom: '16px' }} />
  ...
  const renderKnowledgeBaseView = () => (
    <div style={{ padding: '24px' }}>
      <div style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Title level={3}>Quản lý Knowledge Base</Title>
        <Button onClick={() => setCurrentView('home')}>← Về trang chủ</Button>
      </div>
  ...
  const renderChatView = () => (
    <div style={{ 
      ...
      <div style={{ 
        ...
        <Title level={3}>Chat & Dialogs</Title>
        <Button onClick={() => setCurrentView('home')}>← Về trang chủ</Button>
      </div>
  ...
  const renderEvaluationView = () => (
    <div style={{ padding: '24px', textAlign: 'center' }}>
      ...
      <div className="evaluation-placeholder">
        <ExperimentOutlined />
        <Title level={2} style={{ color: 'black', marginBottom: '16px' }}>RAG Evaluation</Title>
        <Text style={{ fontSize: '16px', opacity: 0.9 }}>
          Tính năng đánh giá hiệu suất RAG.
          <br />
          Chức năng này sẽ giúp đo lường và cải thiện chất lượng câu trả lời của hệ thống.
        </Text>
      </div>
    </div>
  );
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      <Header style={{ 
        ...
        <Title level={3} style={{ margin: 0, color: '#1890ff' }}>DaoMed</Title>
        ...
        >
          ...
          >
            <Select>
              ...
              <Option value="gemini-2.0-flash-thinking-mode">Gemini 2.0 Flash (Thinking Mode)</Option>
          ...
          <Form.Item
            ...
            rules={[
              ...
              { type: 'number', min: 1, max: 8192, message: 'Max tokens must be between 1 and 8192' }
            ]}
          >
          ...
          <Form.Item
            ...
            name="max_chunks"
            rules={[
              { required: true, message: 'Please select max chunks' }
            ]}
          >
            <Slider
              min={1}
